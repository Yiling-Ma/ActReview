# -*- coding: utf-8 -*-
"""
Collect EMNLP 2023 multi-turn review threads.
...
Notes:
- EMNLP 2023 public release contains accepted + opt-in rejected (direct submissions).
- ARR-based submissions are generally NOT included in the EMNLP 2023 OpenReview release.
...
TEXT_FIELDS = ["comment", "response", "text", "message", "rebuttal", "discussion", "review"]
    ...
    for field in CONFIDENCE_KEYS:
        if field in content:
            ...
            if val is not None:
                ...
                return num
    # fallback: sometimes confidence is embedded in other fields
    for k, v in content.items():
        lk = str(k).lower()
        if "confidence" in lk:
            num, _ = parse_numeric_rating(v)
            return num
    return None
...
def join_review_text(content: dict) -> Optional[str]:
    ...
    for k, v in content.items():
        ...
        if isinstance(val, str):
            s = val.strip()
            # filter out pure numeric/short tokens
            if s and not re.fullmatch(r"[0-9.:/,_\-]+", s) and len(s) >= 3:
                header = f"## {k}\n" if k.lower() not in ["review", "summary"] else ""
                texts.append(f"{header}{s}".strip())
    ...
    try:
        with tempfile.NamedTemporaryFile(suffix=".pdf", delete=True) as tmp:
            ...
            section_pattern = re.compile(
                ...
                r"acknowledg(e)?ments?|references|appendix|limitations|"
            ...
            for size, text, page in all_spans:
                ...
                if section_pattern.match(text):
                    ...
                    if key not in seen:
                        seen.add(key)
                        sid += 1
                        sections.append({"section_id": f"s{sid}", "name": name, "page": page})
...
# =========================
# Reply type classification (generalized for EMNLP venues)
# =========================
def classify_reply(reply: dict, venue_id: str) -> str:
    ...
    inv_lowers = [str(x).lower() for x in invs]
    sig_lowers = [str(x).lower() for x in sigs]

    # Decision
    if any("decision" in x for x in inv_lowers):
        return "decision"

    # Meta review
    if any(("meta_review" in x or "metareview" in x) for x in inv_lowers):
        return "meta_review"

    # Official review
    if any(("official_review" in x or x.endswith("/-/review") or x.endswith("/review")) for x in inv_lowers):
        return "review"

    # Author rebuttal / response / official comment
    if any(("author" in x and ("response" in x or "rebuttal" in x or "comment" in x)) for x in inv_lowers):
        return "rebuttal"
    if any(("/authors" in x and ("response" in x or "rebuttal" in x or "comment" in x)) for x in inv_lowers):
        return "rebuttal"

    # signature fallback
    if any(s.endswith("/authors") or "/authors" in s for s in sig_lowers):
        return "rebuttal"
...
def extract_role_and_actor(reply: dict, venue_id: str, event_type: str) -> Tuple[str, Optional[str]]:
    ...
    # For review/comment, infer from signature
    if "area_chair" in sig_lower or "area chairs" in sig_lower:
        return "area_chair", sig
    if "reviewer" in sig_lower:
        return "reviewer", sig
    if sig_lower.endswith("/authors") or "/authors" in sig_lower:
        return "author", sig
...
def build_rating_update_events(client, review_reply: dict, review_event_id: str, venue_id: str) -> List[Dict]:
...
def build_thread_events(
    ...
    for r in replies:
        ...
        if etype == "review":
            ...
            # Decision sometimes in content["decision"] + content["comment"]
...
def _assign_turn_and_phase(events: List[Dict]):
    """Assign event_index, turn, and phase."""
    if not events:
        return
    ...
    for idx, ev in enumerate(events):
        ...
        if etype == "review" and prev_role != "reviewer":
            turn += 1
...
# =========================
def parse_args():
    p = argparse.ArgumentParser(description="Collect EMNLP multi-turn review threads")
    p.add_argument("--years", nargs="+", type=int, default=[2023],
                   help="EMNLP years to collect (default: 2023)")
...
def _fetch_submissions_v1(client_v1, venue_id: str):
    """Fetch submissions via the v1 API (fallback for older venues)."""
    candidate_names = ["Blind_Submission", "Submission", "Paper_Submission", "Abstract_Submission"]
...
def _find_latest_output(output_dir: str, year: int) -> Optional[Path]:
    pattern = f"emnlp{year}_threads_*.jsonl"
    candidates = sorted(Path(output_dir).glob(pattern), key=lambda p: p.stat().st_mtime, reverse=True)
...
def _v1_fetch_all_forum_notes(client_v1, forum_id: str) -> List[dict]:
    """Fetch ALL notes in a v1 forum thread (all nesting)."""
...
def main():
    ...
    if not username or not password:
        raise RuntimeError("Please export OPENREVIEW_USERNAME and OPENREVIEW_PASSWORD")

    client_v2 = openreview.api.OpenReviewClient(baseurl=BASEURL_V2, username=username, password=password)
    client_v1 = openreview.Client(baseurl=BASEURL_V1, username=username, password=password)

    for year in args.years:
        venue_id = f"EMNLP/{year}/Conference"
        sys.stderr.write(f"\n{'='*60}\n")
        sys.stderr.write(f"Collecting EMNLP {year} (venue: {venue_id})\n")
        ...
        if not subs:
            sys.stderr.write(f"No submissions found for EMNLP {year}, skipping.\n")
            sys.stderr.flush()
            continue
        ...
        if not output_path:
            ...
            output_path = os.path.join(args.output_dir, f"emnlp{year}_threads_{timestamp}.jsonl")
        ...
        with open(output_path, open_mode, encoding="utf-8") as fout:
            for sub_idx, sub in enumerate(subs):
                ...
                # Replies: v2 has details["replies"], v1 needs full forum fetch
                if api_version == "v1":
                    try:
                ...
                # Decision convenience extraction
                decision = None
                for ev in thread_events:
                    if ev["event_type"] == "decision":
                        decision = ev["text"]
                        if decision and "\n" in decision:
                            first = decision.split("\n")[0].strip()
                            if any(kw in first.lower() for kw in ["accept", "reject"]):
                                decision = first
                        break
        ...
        print(f"[EMNLP {year}] Saved {stats['total_submissions']} submissions "
